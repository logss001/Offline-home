...
const DeviceCard: React.FC<DeviceCardProps> = ({ device, onToggle, onChangeValue, onDelete, disabled = false }) => {
  ...
  // Animation state for toggle feedback
  ...
  useEffect(() => {
    ...
    if (isMounted.current) {
      setIsAnimating(true);
      const timer = setTimeout(() => setIsAnimating(false), 300); // 300ms flash duration
  ...
  return (
    <div className={`
      relative p-6 rounded-[2rem] backdrop-blur-2xl transition-all duration-300 group overflow-hidden
      border
      ${isFaulty 
      ...
      ${isAnimating ? 'scale-[1.03] !brightness-105 ring-1 ring-emerald-400/30 dark:ring-emerald-500/30' : ''}
    ...
    `}>
      ...
      <div className={`mt-6 flex items-center justify-between ${disabled ? 'pointer-events-none grayscale opacity-50' : ''} relative z-10`}>
        ...
        <button
          ...
          disabled={disabled}
          className={`ml-auto w-14 h-14 rounded-[1.2rem] flex items-center justify-center transition-all duration-300 border backdrop-blur-md active:scale-90 active:shadow-inner ${
            device.isOn 
        ...
        >
          <Power size={22} className={isAnimating ? 'animate-pulse scale-110 duration-75' : ''} />
        </button>
      </div>
    </div>
  );
